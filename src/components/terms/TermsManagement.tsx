...
export default function TermsManagement() {
  ...
  const fetchTerms = async () => {
    try {
      setLoading(true);
      const response = await termsService.getAllTerms('terms');
      ...
      toast.error(error.message || 'Failed to fetch terms and conditions');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    try {
      ...
      if (terms) {
        // Update existing
        await termsService.updateTerms(formData, 'terms');
        toast.success('Terms and Conditions updated successfully!');
      } else {
        // Create new
        await termsService.createTerms(formData, 'terms');
      ...
      setError(error.message || 'Failed to save terms and conditions');
          ...
          <div className="mb-6 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                Manage Terms and Conditions that are displayed in the mobile app
              </p>
            </div>
            {!isEditOpen && (
              <button
                onClick={() => {
                  if (terms) {
                    setFormData({ content: terms.content });
                  }
                  setIsEditOpen(true);
                }}
          ...
          {loading ? (
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
              <div className="p-6 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
                <h2 className="text-xl font-semibold text-gray-800 dark:text-white">
                  {terms ? 'Edit Terms and Conditions' : 'Create Terms and Conditions'}
                </h2>
                <button
                  onClick={() => {
                    setIsEditOpen(false);
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
              <div className="p-12 text-center text-gray-500 dark:text-gray-400">
                ...
                <p className="text-sm mb-6">Create your Terms and Conditions to get started</p>
                <button
                  onClick={() => {
                    setFormData({ content: '' });
                    setIsEditOpen(true);
